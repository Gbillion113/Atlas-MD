...
// ─────────────────────────────────────────────────────────────────────────────
// MONEY WARS HOOKS
// These are self-contained — they read directly from the MWUser mongoose model
// if it exists (loaded by moneywars.js). No top-level import means economy.js
// always loads successfully even if moneywars.js is missing or has an error.
// ─────────────────────────────────────────────────────────────────────────────

const safeApplyDailyBonus = async (id, baseAmount) => {
  try {
    const MWUser = mongoose.models.MWUser;
    if (!MWUser) return { amount: baseAmount, appleNote: "", mult: 1 };
    let mwu = await MWUser.findOne({ id });
...
export default {
  ...
  // ✅ FIX 1: parameter names now exactly match what Core.js passes
  // Core.js passes: prefix, inputCMD, doReact, text, args, mentionByTag,
  //                 pushName, isCreator, isAdmin, modcheck, quoted, etc.
  // Previous version used "isOwner" and "pushname" — neither exist in Core.js
  start: async (Atlas, m, {
    prefix, inputCMD, doReact, text, args,
    mentionByTag, pushName, isCreator,
  }) => {
    ...
    switch (inputCMD) {
      ...
      case "bank": {
        ...
        else if (total >= 1e5) role = "Rich 💰";
        else if (total >= 1e4) role = "Average 💸";
        else if (total >= 1e3) role = "Poor 😢";
        await sendImg(Atlas, m,
          `🏦 *${pushName}'s Bank*\n\n` +
      ...
      case "daily": {
        ...
        // ✅ FIX 2: safe hook — works with OR without moneywars.js
      ...
      case "dep": {
        ...
        if (space <= 0) return m.reply(`❌ Bank is full! Upgrade with *${prefix}capacity*`);
      ...
      case "bankupgrade": {
        ...
        await User.findOneAndUpdate({ id: sender }, {
          wallet: user.wallet - upgrade.cost,
          bankCapacity: user.bankCapacity + upgrade.increase,
        });
      ...
      case "slots": {
        ...
        if (s1 === s2 && s2 === s3) {
          ...
          win = Math.floor(amt * 0.5); msg = "✅ *Small Win!*";
        } else {
          win = -amt; msg = "❌ *No Match!*";
        }
      ...
      case "rob": {
        ...
        // ✅ FIX 3: safe MW golden apple shield check — won't crash without moneywars.js
        ...
        if (hasShield) {
          ...
          return await Atlas.sendMessage(m.from, {
            text: `🛡️ @${target.split("@")[0]} had a shield! Your robbery failed!`,
        ...
        if (chance < 0.4) {
          ...
          await Atlas.sendMessage(m.from, {
            text: `👮 You got caught! Paid $200 fine.\n💰 Wallet: $${formatNum(user.wallet - 200)}`,
            mentions: [sender],
          }, { quoted: m });
        } else if (chance < 0.7) {
          await Atlas.sendMessage(m.from, { text: `😅 You chickened out!`, mentions: [sender] }, { quoted: m });
        } else {
          ...
          await Atlas.sendMessage(m.from, {
            text: `🤑 Robbed @${target.split("@")[0]} for $${formatNum(amount)}!\n💰 Wallet: $${formatNum(user.wallet + amount)}`,
            mentions: [sender, target],
          }, { quoted: m });
        }
        break;
      ...
      case "give": {
        ...
        await Atlas.sendMessage(m.from, {
          text: `💸 Transferred *$${formatNum(amt)}* to @${target.split("@")[0]}!`,
          mentions: [sender, target],
        }, { quoted: m });
        break;
      }
      ...
      case "fish": {
        ...
        let cumulative = 0, caught = catches[0];
        for (const c of catches) { cumulative += c.chance; if (roll < cumulative) { caught = c; break; } }
        const newInv = { ...user.inventory };
        if (caught.item) newInv[caught.item] = (newInv[caught.item] || 0) + 1;
        await User.findOneAndUpdate({ id: sender }, { lastFish: new Date(), inventory: newInv });
        if (caught.item) {
      ...
      case "dig": {
        ...
        const roll = Math.random();
        let cumulative = 0, found = finds[0];
        for (const f of finds) { cumulative += f.chance; if (roll < cumulative) { found = f; break; } }
        const newInv = { ...user.inventory, [found.item]: (user.inventory?.[found.item] || 0) + 1 };
        await User.findOneAndUpdate({ id: sender }, { lastDig: new Date(), inventory: newInv });
      ...
      case "shop": {
        ...
        const list = Object.entries(shopItems)
          .map(([k,v]) => `▪️ *${v.name}* — $${formatNum(v.price)}\n   ${v.description}\n   Buy: *${prefix}buy ${k}*`)
          .join("\n\n");
        m.reply(
          `🛍️ *Economy Shop*\n\n${list}\n\n` +
          `🍎 *MW Golden Apple* — $5,000\n   Doubles daily + 3-turn rob shield\n   Buy: *${prefix}mwapple buy*\n\n` +
          `🍎 *RPG Golden Apple* — $1,000\n   RPG item (sell for $5,000)\n   Buy: *${prefix}rpgbuy goldenapple*`
        );
        break;
      }
      ...
      case "buy": {
        ...
        if (!item) return m.reply(
          `❌ Item not found! See *${prefix}shop*\n\n` +
          `💡 For MW Golden Apple: *${prefix}mwapple buy*\n` +
          `💡 For RPG Golden Apple: *${prefix}rpgbuy goldenapple*`
        );
      ...
      case "inv": {
        ...
        if (!items.length) return m.reply(`🎒 Your inventory is empty!\n\nUse *${prefix}fish*, *${prefix}dig*, or *${prefix}buy* to get items`);
        const list = items
          .map(([k,v]) => `▪️ ${k}: ${v}x${sellPrices[k] ? ` (worth $${formatNum(sellPrices[k])} each)` : ""}`)
          .join("\n");
      ...
      case "rich": {
        ...
        str += `\n👑 Premium leaderboard: *${prefix}richlist*`;
        m.reply(str);
        break;
      }
